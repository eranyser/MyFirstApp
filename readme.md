...
# certify node-gyp (compiler) environment is installed
...
or :
Install [Visual C++ Build Tools](http://go.microsoft.com/fwlink/?LinkId=691126) using the Default Install option. 
...
2)make sure  Python v 2.7 is installed
  and run
...
````bash
 npm run build
````
the artifacts are created in dist folder
...
runs in phantom js, reports to console and outputs html and json to coverage folder
### tdd

````bash  
npm run tdd
````
It opens a chrome for debugging an runs continuously 


#### running to other MP app server
 
 set the IMAGINATE_APP_SERVER environment variable or use
 ````
 npm start -- --env.APP_SERVER youappserver.FQDN
 ````

#### logging

Default log level is WARNING (4) for all packages.
In dev build log will produce file/line/func by default (has performance implications).

To raise all package ID's via script
 ````
 npm run server:debug
 ````
 
Raise specific package ID's
 ````
 (script and parameters) --env.LOG_LEVEL TMR_PKG_WEB_MP_COMMON=7;TMR_PKG_WEB_MP_LM=5
 ````

Raise all package ID's
 ````
 (script and parameters) --env.LOG_LEVEL *7
 ````

Print file/line/func (has performance implications)
 ````
 (script and parameters) --env.LOG_SRC true
 ````
